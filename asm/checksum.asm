; Assembly-optimized CRC32 checksum calculation
; networking library
; Author: kanjore
; Date: 2025
...
section .data
    ...
    crc32_table:
        ...
        dd 0x6c0695ed, 0x1b01a57b, 0x8208f4c1, 0xf50f8457
...
section .text
    global calculate_checksum_asm

; Function: calculate_checksum_asm
; Parameters:
;   rdi: pointer to data (uint8_t *data)
;   rsi: length of data (size_t length)
; Returns:
;   rax: CRC32 checksum (uint32_t)
calculate_checksum_asm:
    push rbx
    push r12
    push r13
    push r14
...
.process_byte:
    ; Get current byte and XOR with current CRC
    movzx rdx, byte [rbx]
    xor dl, al
    and rdx, 0xFF
    
    ; Use byte as index into CRC table
    shl rdx, 2          ; Multiply by 4 (each entry is 4 bytes)
    mov r12d, [crc32_table + rdx]  ; Get table entry
    
    ; Shift CRC right by 8 bits and XOR with table entry
    shr rax, 8
    xor rax, r12
...
; Function: validate_packet_header_asm
; Parameters:
;   rdi: pointer to packet header (packet_header_t *header)
; Returns:
;   rax: 1 if valid, 0 if invalid
global validate_packet_header_asm
validate_packet_header_asm:
    push rbx
    push r12
    push r13
    push r14
    ...
    ; Check type field (must be valid message type)
    mov al, [rdi + 4]
    cmp al, 0x0A
    ja .invalid

    ; Check sequence number (should not be zero for valid packets)
    mov ebx, [rdi + 5]
    test ebx, ebx
    jz .invalid
    ...
    ; If we get here, header is valid
    mov rax, 1
    jmp .done
